...
class BlocklistControllerTest {
  ...
  private lateinit var serviceMock: CachedBlocklistService

  @BeforeEach
  fun setup() {
    serviceMock = Mockito.mock(CachedBlocklistService::class.java)
  ...
  fun `isIpInBlocklist of an ip should return true when service returns a blocklist containing the ip`() {
    Mockito.`when`(serviceMock.getCachedIpBlocklist()).thenReturn(setOf("2.2.2.2", "1.1.1.1", "3.3.3.3"))
  ...
  fun `isIpInBlocklist of an ip should return false when service returns a blocklist without the ip`() {
    Mockito.`when`(serviceMock.getCachedIpBlocklist()).thenReturn(setOf("2.2.2.2"))
